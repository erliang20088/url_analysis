...
public class ObjectIoUtil { 
	...
	public static Object readObject(String file_path) {
		...
		try {
			// int bytes_length =
			// SystemParas.cache_relation_load_bytes_length;// 初始化字节数组的空间大小
			int bytes_length = 0;
	...
	public static boolean writeObject(String file_path, Object bloom) {
		...
		try {
			...
			output
					.write(ObjectAndByteArrayConvertUtil
							.ObjectToByteArray(bloom));
			output.close();
		} catch (Exception e) {
			e.printStackTrace();
			return false;
	...
	public static boolean writeObjectByFileOutputStream(String file_path,
			Object bloom) {
		FileOutputStream output = null;
		ObjectOutputStream objectOutput = null;
		// 判断父路径是否存在，若存在则不做创建操作，若不存在，首先创建父路径
